...
const megare = /\[(?<time>\d{4}-\d{2}-\d{2} \d{2}:\d{2})] (?<event>Guard #(?<id>\d+) begins shift|wakes up|falls asleep)/i;
export const format = input =>
  input
    .split("\n")
    .map(i => {
      const [, time, event, id] = megare.exec(i);
      return {
        time: dateFns.parse(time),
        event,
        id
      };
    })
...
export default format(`[1518-09-16 23:57] Guard #1889 begins shift
